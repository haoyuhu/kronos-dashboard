...
class KronosForecastingApp:
    ...
    def _ensure_model_loaded(self) -> None:
        ...
        if not self.model_loaded and not self.mock:
            logger.info(
                f"Loading Kronos model (size={self.model_config.model_size}, device={self.model_config.device})..."
            )
    ...
    def _run_mock_mode(self) -> Dict[str, dict]:
        ...
        for chart_file in chart_files:
            ...
            metrics[symbol] = {
                ...
                "chart_path": str(
                    (self.repo_path / "static" / "chart" / f"{symbol}.png").relative_to(
                        self.repo_path
                    )
                ),
            }
    ...
    def _run_inference_mode(self) -> Dict[str, dict]:
        ...
        for source_name in self.data_manager.available():
            ...
            for symbol in symbols:
                ...
                # Fetch data
                total_points = (
                    self.common_config.hist_points + self.common_config.vol_window
                )
                df_full = self.data_manager.fetch(
                    source_name, symbol, interval, total_points
                )
                ...
                if result:
                    metrics[symbol] = {
                        ...
                        "chart_path": str(
                            result.chart_path.relative_to(self.repo_path / "docs")
                        ),
                    }
...
Examples:
  ...
  %(prog)s --schedule         # Run continuously on schedule
    ...
    parser.add_argument(
        "--schedule", action="store_true", help="Run continuously on schedule"
    )
