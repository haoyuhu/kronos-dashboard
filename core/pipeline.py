...
def make_prediction(df, predictor, pred_horizon, n_predictions, interval):
    ...
    new_timestamps_index = pd.date_range(
        start=start_new_range, periods=pred_horizon, freq=freq
    )
...
def calculate_metrics(hist_df, close_preds_df, v_close_preds_df, vol_window):
    ...
    print(
        f"Upside Probability (24h): {upside_prob:.2%}, Volatility Amplification Probability: {vol_amp_prob:.2%}"
    )
...
def create_plot(
    ...
    try:
        ...
        pred_time = pd.to_datetime(
            [
                last_hist_time + timedelta(**{time_unit: i + 1})
        ...
        ax1.set_title(
            f"{symbol} Probabilistic Price & Volume Forecast (Next {pred_horizon} {title_horizon_unit})",
            fontsize=16,
            weight="bold",
        )
